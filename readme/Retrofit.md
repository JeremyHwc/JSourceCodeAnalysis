...
| 类名                | 说明                                                         |      |
| ------------------- | ------------------------------------------------------------ | ---- |
| Retrofit            | 作为该框架的统一入口，通过Retrofit可以对这个参数进行统一的配置，配置方式是通过Retrofit当中的一个内部类Builder来进行构建的。 |      |
| ServiceMethod       | 对应的本地定义的接口类中的方法，通过动态代理模式，将定义好的接口以及接口当中的参数转换为Http请求。解析方法当中的注解，生成我们需要的Request对象 |      |
| CallAdapterFractory | 用于生产CallAdapter，其中CallAdapter会将请求封装成OkHttpCall ???????????????????????? |      |
| ConverterFactory    | 用于生产数据转换器Converter，默认情况下是通过Converter将Response转换为Java对象。在Retrofit当中，默认使用GSON来解析数据，所以在默认情况下，ConverterFactory就是GsonConverterFactory。 |      |
| CallFactory         | 用于创建Call请求类，在Retrofit当中，请求都会被封装成Call类。 |      |
| OkHttpCall          |                                                              |      |
| CallAdapter         |                                                              |      |
| Converter           |                                                              |      |
| callbackExecutor    |                                                              |      |

![](imgs/retrofit/retrofit_flow.png)



## Retrofit概述

![](imgs/retrofit/retrofit_arch.png)

Retrofit并不是一个网络请求框架，因为其真正的网络请求是由okhttp完成的，其只是对OkHttp进行了一个封装。

总之，App应用程序通过Retrofit请求网络，实际上是使用Retrofit接口层封装请求参数，之后由OkHttp完成后续请求操作。在服务端返回数据之后，OkHttp将原始的结果交给Retrofit,Retrofit根据用户的需求对结果进行解析。
...
- 创建用于描述网络请求的接口(Retrofit是将每一个http请求抽象成了java接口，通过注解来描述配置网络请求的参数，内部实现原理

  就是用动态代理机制将我们的注解翻译成一个一个的http请求，最后由线程池执行http请求)
...
![](imgs/retrofit/static_proxy.png)
...
- 无侵入
- 通俗：增强方法
- 相比于静态代理，很方便对被代理类的函数做统一处理，不用修改每一个代理类的函数。

**动态代理：**代理类在程序运行时创建的代理方式。

**动态代理两种模式**

分别包括jdk动态代理和CGLIB动态代理。其中jdk动态代理需要自行定义接口，而CGLIB是可以直接修改字节码来做。由于jdk动态代理是由Java内部的反射机制来实现的，所以jdk动态代理反射机制在生成类的过程中比较高效。
...
// 网络请求适配器工厂的集合，适配器就是把call对象转换成其他类型，比如：把call转换成RxJava平台的call，
// CallAdapter.Factory就是用来生产CallAdapter
...
public static final class Builder {
    // 表示Retrofit适配的平台，包括Android、Java 8等，默认情况都是Android平台
    private final Platform platform;
    // 请求网络的Okhttp工厂，在默认情况下Retrofit使用的是OkHttp来进行网络请求的，所以默认值就是OkHttpClient
    private @Nullable okhttp3.Call.Factory callFactory;
    // 网络请求的Url
    private @Nullable HttpUrl baseUrl;
    // 将Response转换成Java对象，主要起到一个数据转换的作用。默认情况下使用的是GsonConverterFactory
    private final List<Converter.Factory> converterFactories = new ArrayList<>();
    // 网络请求适配器工厂集合，将Call对象转换成其他类型能用的其他请求
    private final List<CallAdapter.Factory> callAdapterFactories = new ArrayList<>();
    // 执行回调的线程池，默认是MainThreadExecutor
    private @Nullable Executor callbackExecutor;
    // 表示是否需要立即解析接口中的方法，用于动态代理解析定义好的接口的注解标志位
...
- baseUrl必须以“/”结尾。其中baseUrl和Endpoint的组合关系见baseUrl方法注释
- converter就是数据转换器
...
public Retrofit build() {
      ...
      // callFactory用于产生call的工厂，如果callFactory为空，Retrofit默认使用OkHttpClient进行网络请求
...
![](imgs/retrofit/rxjava_calladapter_factory.png)
